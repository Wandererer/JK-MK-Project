...
public class ground2_move : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (z < -2000)
        {
            GameObject ground = GameObject.Find("Terrain-1");

            if (ground == null)
            {
                ground = Instantiate(ground1move);
               // Debug.Log("ground1move has been Instantiated");
                ground.name = "Terrain-1";
